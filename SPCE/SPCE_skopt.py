...
class StoppingCriterion(EarlyStopper):
    """
    A custom stopping criterion for early stopping of optimisation algorithms ('gp_minimize').

    This class inherits from the EarlyStopper class from skopt.callbacks.
    It defines a stopping criterion based on the difference
    between the best and worst function values of the n_best iterations of optimization.

    Inputs:
        delta (float): The threshold value for the difference.
        n_best (int): The number of function values of last iterations to consider for calculating the threshold difference.

    Methods:
        _criterion(result): Private method to check whether the stopping criterion is met based on the optimization result.
    """
    
    def __init__(self, delta, n_best):
...
class StochasticPCE:
    def __init__(self, X, y, dist_obj, D, p_values, q_values, sim_name):
        ...
        self.Ns = 20 # the number of points for the warm-start strategy to estimate c
    ...
    def gp_minimize_sigma(self, objective_f, sigma_space, n_jobs=None, verbose=False):
        """
        Uses the Gaussian Process-based Bayesian optimization algorithm (gp_minimize) 
        to find the optimal value of 'sigma' that minimizes the objective function provided. 
        
        'gp_minimize' is a global optimization method from skopt that efficiently searches
        for the minimum of the objective function by iteratively selecting the next evaluation 
        point based on previous evaluations.

        The objective function should return a score * -1 to get maximization.

        The function returns negative CV score corresponfing to 'optimal_sigma'.
        """

        res = gp_minimize(func=objective_f, 
                          dimensions=sigma_space,
                          initial_point_generator="lhs",
                          acq_func = "EI",
                          n_calls=100, 
                          n_initial_points=20, 
                          callback = StoppingCriterion(delta=0.01, n_best=20),
                          n_jobs=n_jobs)

        optimal_sigma = res.x[0]
        associated_l  = res.fun

        if verbose==True:
            # plot_convergence(res)
            return optimal_sigma, -associated_l, res
    ...
    def optimize_sigma(self, sigma):
        ...
        For maximization using 'gp_minimize', returns a negative CV score.
        """
        # for gp_minimize
        sigma = sigma[0] 
        ...
        while iz < Nz:
            ...
            while ip < Np:
                while iq < Nq:
                    ...
                    print("sqrt of eps_loo in main function:", np.sqrt(self.eps_loo))

                    sigma_space = [Real(0.1 * np.sqrt(self.eps_loo), 1 * np.sqrt(self.eps_loo), name="sigma")]

                    start_time = time.time()
                    opt_sigma, opt_cv_score, res = self.gp_minimize_sigma(objective_f=self.optimize_sigma, 
                                                                          sigma_space=sigma_space,
                                                                          n_jobs=10, 
                                                                          verbose=True)

                    print("Bayesian optimization took:  - %s seconds -" % (time.time() - start_time))

                    if not os.path.exists(f"bayesian_opt_verbose/{self.sim_name}"):
                        os.makedirs(f"bayesian_opt_verbose/{self.sim_name}")
                        

                    print("List of sigma values tried : {}".format(res.x_iters))
                    print("List of corresponding CV scores : {}".format(res.func_vals))
                    print("Sorted CV scores : {}".format(np.sort(res.func_vals)))

                    plot_convergence(res)
                    plt.title(f"Convergence plot D: {self.D[iz]}, p={self.p_values[ip]}, q={self.q_values[iq]}")
                    if not os.path.exists(f"bayesian_opt_verbose/{self.sim_name}/bo_D_{self.D[iz]}_p_{self.p_values[ip]}_q_{self.q_values[iq]}.png"):
                        plt.savefig(f"bayesian_opt_verbose/{self.sim_name}/bo_D_{self.D[iz]}_p_{self.p_values[ip]}_q_{self.q_values[iq]}.png", 
                                bbox_inches="tight")
                    plt.show()
                    
                    saveObject = (res.x_iters, res.func_vals)
                    if not os.path.exists(f"bayesian_opt_verbose/{self.sim_name}/bo_D_{self.D[iz]}_p_{self.p_values[ip]}_q_{self.q_values[iq]}.pickle"):
                        with open(f"bayesian_opt_verbose/{self.sim_name}/bo_D_{self.D[iz]}_p_{self.p_values[ip]}_q_{self.q_values[iq]}.pickle", 
                                 "wb") as output_file:
                            cPickle.dump(saveObject, output_file)
